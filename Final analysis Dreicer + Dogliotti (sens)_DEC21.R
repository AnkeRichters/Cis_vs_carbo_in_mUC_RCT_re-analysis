...
# Adjusting time and censoring
do.total$Event_sx<-ifelse(do.total$Event==0, 1, do.total$Event)              # Turn censored events into death events
dr_cis <- dr.total[which(dr.total$arm==1 & dr.total$Event==1),1]
dr_car <- dr.total[which(dr.total$arm==2 & dr.total$Event==1),1]
set.seed(12345)
impute_cis<-(sample(dr_cis,55,replace=T))
impute_car<-(sample(dr_car,55,replace=T))
impute_ciscar<- c(impute_cis,impute_car)
do.total$Time_sx<-impute_ciscar
do.total$Time_sx<-ifelse(do.total$Event==1, do.total$Time, do.total$Time_sx)              # Turn censored events into death events
...
#PLOT 1
lay3<-rbind(c(3,3,5,5,6,6),
            c(3,3,5,5,6,6))
t <- textGrob(" ")
plotA3<-grid.arrange(plot3$plot,plot5$plot,plot6$plot, layout_matrix=lay3)
ggsave(filename="Plot1_rev_random-imputed.jpg", plot=plotA3, path=path3, width=20, height=6, units=c("cm"), dpi=300, limitsize=F)
...
rmst2(dr.total$Time, dr.total$Event, dr.total$arm, tau=24)
rmst2(dr.total$Time, dr.total$Event, dr.total$arm, tau=36)
...
rmst2(df2$Time_sx, df2$Event_sx, df2$armx, tau=12)
